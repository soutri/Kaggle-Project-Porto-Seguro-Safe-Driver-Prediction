...
fitted_results_model = predict(logistic.fit, newdata = as.data.frame(test_train_data), type = 'response')

fit = rep(0,length(fitted_results_model))
fit[fitted_results_model > 0.5] = 1

mean(fit == test_train_label) * 100

prediction = predict(logistic.fit, newdata = as.data.frame(test), type = 'response')

result = cbind(test_id,prediction)
colnames(result) = c("id","target")

as.data.frame(result) %>% write_csv('submit.csv')
